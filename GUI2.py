...
class ssr_window(ctk.CTkFrame):
    def __init__(self, parent):
        ...
        self.sample_import_param = ctk.CTkButton(self.parameters, border_width=1, border_color="black", text_color="black", text = "Write Samplesheet", command = self.set_sample_import_params)
        self.sample_import_param.grid(row=1, column=0, padx=20, pady=(10, 10))
    ...
    def check_status(self):
        ...
        nrsamples = int(self.sample_params_import[2]) - int(self.sample_params_import[1])
        #nrsamples = 9
    ...
    def set_sample_import_params(self):
        ...
        write_samplesheet_button = tk.Button(newWindow, text="Write Samplesheet", command = lambda x = sample_entries, y = sequence_entries : self.set_samplesheetname_name(x, y))
        write_samplesheet_button.grid(column=0, row=2, sticky="sw", padx=15, pady=4)
    
    def set_samplesheetname_name(self, x, y):
        print("Writing Samplesheet:")
        
        self.sample_params_import[0] = int(x[0].get())
        self.sample_params_import[1] = int(x[1].get())
        self.sample_params_import[2] = int(x[2].get())
        
        self.sequence_params_import[0] = int(y[0].get())
        self.sequence_params_import[1] = int(y[1].get())
        self.sequence_params_import[2] = int(y[2].get())
        ...
        enter = ctk.CTkButton(newWindow, border_width=1, border_color="black", text_color="black", text = "Enter", command = lambda a = x, b = y, c = samplesheet_entry : self.writesamplesheet(a, b, c))
        enter.grid(row=2, column=0, padx=20, pady=(20, 10))
    
    def writesamplesheet(self, a, b, c):
        ...
        if (not(".csv" in samplesheet)):
            newWindow = tk.Toplevel(self)
            newWindow.title("!!")
            newWindow.grid_rowconfigure(0, weight=1)
            newWindow.grid_columnconfigure(0, weight=1)
            logo_samplesheet = ctk.CTkLabel(newWindow, text="Samplesheetname has no .csv at the end", font=ctk.CTkFont(size=20, weight="bold"))
            logo_samplesheet.grid(row=0, column=0, padx=20, pady=(20, 20))
            raise Exception("Samplesheetname must habe .csv at the end")
        
        sample_columnr = int(a[0].get())
        sample_rownr_begin = int(a[1].get())
        sample_rownr_end = int(a[2].get())
        
        sequence_columnr = int(b[0].get())
        sequence_rownr_begin = int(b[1].get())
        sequence_rownr_end = int(b[2].get())
    ...
    def parse_parameter_file(self):
        ...
        if (os.path.exists(paramter_file_path)):
            ...
            calc_param_list = ["2", "290", "310 600", "0.7", "20"]
            param_list_special = ["diploid", "windows", "Default"]
    ...
    def write_params(self, entries):
        ...
        with open("Parameter.txt", 'w') as file:
            ...
            file.write("ReferenceFile = " + os.path.normpath(self.reference_file) + " \n")
