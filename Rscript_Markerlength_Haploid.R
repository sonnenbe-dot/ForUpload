...
for (file in dir(path,full.names=TRUE)) {
  ...
  if (file.size(file) == 0 ) {   
    ...
    i=i+1
    plot(1,ylab="empty or nearly empty file", xlab="", xlim=c(xlim_lower_smallest, xlim_upper_largest), ylim=c(0,105), type="h", main=output, sub=paste("Total read count is",sep=" "))
  ...
  filein<-filein[order(filein$V2),]  ##everywhere filein instead of filein1
  ...
  if ((maxcount > 20) & (n > 1)) {                               #call mincount variable from command line input here
          #first case one length highly abundant - homozygous 
              if ( maxPeak/maxcount >= 0.7)  { #
                  plot(filein[,1],100*filein[,2]/maxcount,xlab="fragment length",ylab="absolute read count",type="h", main=output, sub=paste("Total read count is",maxcount,sep=" "),lwd = 5, xlim=c(xlim_lower_smallest, xlim_upper_largest),ylim=c(0,105), col=ifelse(filein$V1==maxleng, "black", "black"))
                  ...
                  dat<-rbind(dat, foo)
                      ...
                      plot(filein[,1],100*filein[,2]/maxcount,xlab="fragment length",ylab="absolute read count",type="h", main=output, sub=paste("Total read count is",maxcount,sep=" "),lwd = 5, xlim=c(xlim_lower_smallest, xlim_upper_largest),ylim=c(0,105), col=ifelse(filein$V1==maxleng | filein$V1==maxleng2,"black", "black"))
        ...
        plot(1,ylab="empty file", xlab="", xlim=c(xlim_lower_smallest, xlim_upper_largest), ylim=c(0,105), type="h", main=output, sub=paste("Total read count is",sep=" "))
        mtext("empty file",side=3,cex=0.8)
        foo<-data.frame(samplename = samplename, marker = paste(markername, "1", sep="_"), maxleng = "empty")
        dat<-rbind(dat, foo)
        foo<-data.frame(samplename = samplename, marker = paste(markername, "2", sep="_"), maxleng = "empty")
     ...
     plot(filein[,1],100*filein[,2]/maxcount,xlab="fragment length",ylab="absolute read count",type="h", main=output, sub=paste("Total read count is",maxcount,sep=" "),lwd = 5, xlim=c(xlim_lower_smallest, xlim_upper_largest),ylim=c(0,105), col="grey")
