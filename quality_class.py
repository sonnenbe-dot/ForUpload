...
def main():
    ...
    Sequence_inst = class_quality(project_folder, output_folder, primerfile, samplesheet, minCount, alpha, datatype, unique_name, par_os, allele_list_path, textbox_checkbox)
    Sequence_inst.print_input_paths()
    print()
    Sequence_inst.get_Marker_Sample_Alleles_length_diploid()
    print()
...
class class_quality():
    def __init__(self, project_folder, output_folder, primerfile, samplesheet, minCount, consThreashold, datatype, unique_name, par_os, allele_list_path, textbox_bool):
        self.old_workspace_path = os.getcwd()
        
        self.project_folder_path = os.path.normpath(self.old_workspace_path + "/" + project_folder)
        self.project_folder = project_folder
        
        
        
        new_workspace_folder_path = os.path.normpath(self.project_folder_path + "/" + output_folder)
        ...
        if os.path.exists(new_workspace_folder_path):
            ...
            self.new_workspace_folder = output_folder
        else:
        ...
        primerfile_path = os.path.normpath(self.old_workspace_path + '/' + primerfile)
        ...
        samplesheet_path = os.path.normpath(self.old_workspace_path + '/' + samplesheet)
    ...
    # get marker/loci list, sample list from allele length codominat csv matrix, also get sequences per allelelength from separatout folder
    #This function only calls when the datatype is diploid (2 alleles per loci/sample combination)
    def get_Marker_Sample_Alleles_length_diploid(self):
        folder_path = self.separatout_path
        separatout_filetype = self.getFileType(folder_path) #Get the filetype of separatout files (should be fastq or fasta)
        ...
        with open(self.matrix_file_path) as allele_matrix:
            for line in allele_matrix:
                ...
                if (sample == 'samplename'): #we are in the first line which is the header
                    loci_names = line_splitted[1:]
                    loci_names_trimmed = ['_'.join(loci_name.split('_')[0:2]) for loci_name in loci_names] #trimming away the _1 and _2 parts of each loci name
                    #The join function joins all trimmed string elements of the loci_names list and adds '_' inbetween which is basically nothing
                    self.loci_names = list(set(loci_names_trimmed)) #Make all loci_names unique, we dont need duplicates!
                    self.loci_names.sort() 
                    self.analysis["Number of loci"] = str(len(self.loci_names))
                else:
                    self.sample_names.append(sample)
                    allele_lengths = line_splitted[1:]
                    allele_lengths = [allele_length.strip('"').replace('empty', '0').replace('too little reads', '0').split('_')[0] for allele_length in allele_lengths] #Using split('_')[0] to also eliminate the _man check string
                    ...
                    if not(len(self.loci_names) == 0):
                        i = 0
                        #iterating through all loci and all allele lengths for each loci
                        for locus in self.loci_names:
                            allele_length_dict[str(sample)+ "_" + str(locus)] = {}
                            #locus = self.loci_names[i]
        ...
        with open (con_sequence_file_path, 'w') as cons_file:
            with open(frequency_file_path, 'w') as freq_file:
                ...
                for position, nuc in data.items(): #nuc is a list of nucleotides for the position rec for all sequences in the file
                    ...
                    for n in ['A', 'C', 'G', 'T']:
                        freq = nuc.count(n)/len(nuc) # get frequency from each nucleotide #nr_seq should be the same value as len(nuc)
        ...
        for locus in self.loci_names:
            ...
            with open(output_fasta_path, 'w') as output_fasta:
                for file in os.listdir(input_path):
                    if file.startswith(locus) and file.endswith('.fasta'):
    ...
    def correctSequences(self, fasta, AlleleDir, outFasta): #the function is called for each fasta file in the consensus_together folder
        ...
        for name, seq in self.parsing_fastq_fasta(fasta).items(): #Each fasta file in the consensus_together folder has only 1 header and 1 sequence, because they are all put together.
            if seq != '':
                ...
                positions[(name, seq)] = self.get_Ns_records(seq) # key is a tuple with the name of the sample and the sequence, while the value is a list with the postions conataing Ns
    ...
    def AlleleCall(self):
        ...
        if (self.denovo_bool == 0):
            ...
            self.AlleleCall_listbased(corrected_folder_path, allelecall_output_folder_path)
        else:
    ...
    def AlleleCall_listbased(self, corrected_folder_path, allelecall_output_folder_path):
        ...
        sample_loci_matrix = self.CallAlleles(NewLocusAlleleList, corrected_folder_path, allelecall_output_folder_path)
    ...
    def parse_primerfile(self):
        ...
        with open(self.primerfile_path) as primerfile:
            for line in primerfile:
                ...
                if list(set(line_list)) != [""]:
                    primers = line_list[1:] # primer sequences
    ...
    def CallAlleles(self, locusAlleleList, corrected_folder_path, allelecall_output_folder_path):
        ...
        with open(output_allelelist_path, 'w') as allele_list:
            for file in os.listdir(corrected_folder_path): #each file represents the information for 1 unique locus
                ...
                names_alleles = self.Define_genotypes(alleles_dict, parsed_fasta)
